...
//    }
    ...
    @Override
    public List<LogSummaryDTO> getCustomLogSummary(String email) {
        String queryStr = "SELECT dataLabel, dateRange, " +
                "(SELECT COUNT(*) " +
                "FROM logs_summary_daily " +
                "WHERE email = :email AND stage = 'INGRESS' AND created_on >= dateRange AND created_on < DATE_ADD(dateRange, INTERVAL 1 DAY)) AS ingressLogCount, " +
                "(SELECT COUNT(*) " +
                "FROM logs_summary_daily " +
                "WHERE email = :email AND stage = 'EGRESS' AND created_on >= dateRange AND created_on < DATE_ADD(dateRange, INTERVAL 1 DAY)) AS egressLogCount " +
                "FROM logs_summary_daily " +
                "WHERE email = :email " +
                "GROUP BY dateRange " +
    ...
    public List<LogSummaryDTO> getCustomLogSummaryLastTenHours(String email) {
        String queryStr = "SELECT dataLabel, hourRange, " +
                "(SELECT COUNT(*) " +
                "FROM logs_summary_hourly " +
                "WHERE email = :email AND stage = 'INGRESS' AND created_on >= hourRange AND created_on < DATE_ADD(hourRange, INTERVAL 1 HOUR)) AS ingressLogCount, " +
                "(SELECT COUNT(*) " +
                "FROM logs_summary_hourly " +
                "WHERE email = :email AND stage = 'EGRESS' AND created_on >= hourRange AND created_on < DATE_ADD(hourRange, INTERVAL 1 HOUR)) AS egressLogCount " +
                "FROM logs_summary_hourly " +
                "WHERE email = :email " +
                "GROUP BY hourRange " +
                "ORDER BY hourRange DESC " +
                "LIMIT 24";
